...
StrategyGetBuffer(void)
{
	...
	while(StrategyControl->firstFreeBuffer != FREEPREVIOUS_END_OF_LIST){
		buf = &BufferDescriptors[StrategyControl->firstFreeBuffer];
		StrategyControl->firstFreeBuffer = buf->freePrevious;
	}
	...
	for (;;)
	{
		...
		StrategyControl->nextVictimBuffer = buf->freeNext;
		...
		if(StrategyControl->nextVictimBuffer == FREENEXT_END_OF_LIST){
			StrategyControl->nextVictimBuffer = StrategyControl->firstFreeBuffer;
		}
		...
		*/
		...
		else
		{
			...
			if(StrategyControl->nextVictimBuffer != FREENEXT_END_OF_LIST){
				...
				BufferDescriptors[StrategyControl->nextVictimBuffer].freePrevious = buf->freePrevious;

				if(buf->freePrevious != FREEPREVIOUS_END_OF_LIST){
					BufferDescriptors[buf->freePrevious].freeNext = StrategyControl->nextVictimBuffer;
				}
				
				BufferDescriptors[StrategyControl->firstFreeBuffer].freePrevious = FREEPREVIOUS_END_OF_LIST;
				...
				buf->freePrevious = BufferDescriptors[StrategyControl->lastFreeBuffer].buf_id;
